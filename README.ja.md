...
注記（プラットフォーム）: 本プロジェクトは主に Radxa ROCK 5B 上で開発・検証しています。I2C を有効にし依存関係を整えれば、Raspberry Pi 4/5 でも動作する可能性があります（バス番号やデバイスノードが異なる場合があります）。
...
OBSから直接FLV(TCP)で送信し、プレイヤー側で受信・再生する手順です。
...
2) OBS 設定（録画タブ → カスタム出力(FFmpeg)）
- コンテナ: flv
- 映像: H.264（x264 など）
- 音声: AAC
- 出力先 URL: `tcp://<受信機IP>:5004`

注意:
- RTMP ではなく「カスタム出力(FFmpeg)」を選んでください。
- 終了時は受信側がEOFで終了します（systemd運用の場合は自動再起動されます）。
- サービス運用時は `MODE=flv`、`PORT=5004` に設定してください（例は `README.systemd-ja.md` 参照）。
