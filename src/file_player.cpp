...
int main(int argc, char* argv[]) {
    const std::string usage = 
        ...
        "    --record-mp4 PATH: Record emulator output to an MP4 file\n"
    ...
    return common_main_runner(usage, argc, argv,
        [](const std::string& video_path, const DisplayConfig& config, ScalingMode scaling_mode, int min_threshold, int max_threshold, bool debug, bool loop, const std::string& record_mp4_path) {
            ...
            auto play_once = [&](void) -> int {
                if (config.type == "emulator") {
                    return play_video_stream_emulator(video_path, config, stop_flag, scaling_mode, min_threshold, max_threshold, debug, record_mp4_path, !record_mp4_path.empty());
                } else {
            ...
            if (config.type == "emulator") {
                if (loop) {
                    ...
                    while (!stop_flag && !g_should_exit) {
                        loop_count++;
                        if (debug) std::cerr << "[file_player] emulator loop iteration " << loop_count << std::endl;
                        int rc = play_once();
                        if (debug) std::cerr << "[file_player] play_once returned rc=" << rc << std::endl;
                        if (stop_flag || g_should_exit) break;
