...
Copy of led_sequencer.py with a batch test mode:
 - Cycles through segments A..DP
 - Displays the same segment on all digits simultaneously

Usage: python led_batch_test.py [--layout LAYOUT] [--delay 0.5]
"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="7-seg LED batch test: cycle segments on all digits")
    ...
    try:
        ...
        while True:
            # Batch mode: for each segment, set that segment on ALL digits simultaneously
            for seg in SEGMENTS:
                ts = time.strftime("%Y-%m-%d %H:%M:%S")
                if mode == "print":
                    print(f"{ts} - ALL DIGITS SEG {seg} ON")
                else:
                    seg_bit = segment_name_to_bit(seg)
                    grid = [seg_bit] * len(digits)
                    ok = apply_grid_to_modules(bus, layout, grid, error_on_fail=False)
                    print(f"{ts} - ALL DIGITS SEG {seg} -> i2c grid write seg=0x{seg_bit:02X} ok={ok}")
                time.sleep(delay)
        ...
        for tca in tca_list:
            ...
            if tca.get("rows"):
                ...
                for row_id, row_cfg in rows.items():
                    ...
                    if not address_grid:
                        continue
                    if channel_grid_width := None:
                        pass
                    # reuse original implementation logic for mapping
                    # fall back to apply original channel-based handling
            else:
                for channel, address_grid in (tca.get("channels", {}).items() if isinstance(tca.get("channels", {}), dict) else []):
                    ...
                    if str(channel) == "-1" and channel_grid_width == 1 and channel_grid_height > 1:
                        try:
