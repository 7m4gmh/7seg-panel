...
Note (platform): Developed and tested primarily on Raspberry Pi 3 and Radxa ROCK 5B. It also works on Raspberry Pi 4/5 if I2C is enabled and dependencies are installed (bus numbers/device names may differ).
...
注記（プラットフォーム）: 本プロジェクトは主に Raspberry Pi 3 および Radxa ROCK 5B 向けに作成・検証しています。I2C を有効化し依存関係を整えれば Raspberry Pi 4/5 でも動作します（バス番号やデバイス名が異なる場合があります）。
